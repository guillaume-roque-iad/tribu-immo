...
The presentation form uses the existing Apps Script contact endpoint. It transmits the selected session with the message; server success means the request was received, NOT that a confirmation email or Calendar invitation was sent. Automatic confirmation requires an authenticated update to the existing Apps Script project. Do not expose Meet links in static assets or claim that delivery is active before verification. Calendar downloads are reminders, not confirmed registrations. Never send fake test registrations.
